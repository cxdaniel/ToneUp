...
Deno.serve(async (req)=>{
  ...
  const { user_id, inds, dur = 60, acts = null } = await req.json();
  if (!inds || !inds) {
  ...
  try {
    ...
    debug(`7. 生成材料:${JSON.stringify(cozeOutput)}`, 2);
    // 8. 获取可分配的活动库
    ...
    debug(`9. 生成计划:${JSON.stringify(planData)}`, 2);
    /** 待保存材料库数据 */ // 
    ...
    // const saved_plan = runSaveTransactionJS({
    //   user_id,
    //   currentLevel,
    //   focusIndicators,
    //   cozeOutput,
    ...
    // });
    // 完成请求
    return new Response(JSON.stringify(saved_plan), {
      headers: {
...
/** 创建并保存quizes */ //
async function _saveQuizesData({ planData, cozeOutput }) {
...
/** 保存活动实例到数据库的函数 */ //
async function _savePracticesData({ planData, saved_quizes }) {
  let index = 0;
